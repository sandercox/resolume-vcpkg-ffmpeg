vcpkg_from_github(
	...
	# Do not remove this patch. It is required by chromium
...
vcpkg_add_to_path(
	PREPEND
	"${CURRENT_HOST_INSTALLED_DIR}/manual-tools/ffmpeg-bin2c"
)
...
set(OPTIONS
	"--enable-pic --disable-doc --enable-runtime-cpudetect --disable-autodetect"
)
...
elseif(VCPKG_TARGET_IS_UWP)
	string(APPEND
		OPTIONS
		" --target-os=win32 --enable-w32threads --enable-d3d11va --enable-d3d12va --enable-mediafoundation"
	)
...
elseif(VCPKG_TARGET_IS_OSX)
	string(APPEND
		OPTIONS
		" --target-os=darwin --enable-appkit --enable-avfoundation --enable-coreimage --enable-audiotoolbox --enable-videotoolbox"
	)
elseif(VCPKG_TARGET_IS_IOS)
	string(APPEND
		OPTIONS
		" --enable-avfoundation --enable-coreimage --enable-videotoolbox"
	)
...
if(VCPKG_DETECTED_CMAKE_CXX_COMPILER)
	get_filename_component(CXX_path
...
if("nvcodec" IN_LIST FEATURES)
	...
	set(OPTIONS
		"${OPTIONS} --enable-cuda --enable-nvenc --enable-nvdec --enable-cuvid --enable-ffnvcodec"
	)
else()
	set(OPTIONS
		"${OPTIONS} --disable-cuda --disable-nvenc --disable-nvdec  --disable-cuvid --disable-ffnvcodec"
	)
endif()
...
if("qsv" IN_LIST FEATURES)
	set(OPTIONS
		"${OPTIONS} --enable-libvpl --enable-encoder=h264_qsv --enable-decoder=h264_qsv"
	)
	set(WITH_VPL ON)
else()
...
if(VCPKG_TARGET_IS_UWP)
	set(ENV{LIBPATH}
		"$ENV{LIBPATH};$ENV{_WKITS10}references\\windows.foundation.foundationcontract\\2.0.0.0\\;$ENV{_WKITS10}references\\windows.foundation.universalapicontract\\3.0.0.0\\"
	)
	string(APPEND OPTIONS " --disable-programs")
	string(APPEND
		OPTIONS
		" --extra-cflags=-DWINAPI_FAMILY=WINAPI_FAMILY_APP --extra-cflags=-D_WIN32_WINNT=0x0A00"
	)
	string(APPEND
		OPTIONS
		" --extra-ldflags=-APPCONTAINER --extra-ldflags=WindowsApp.lib --extra-ldflags=dxguid.lib"
	)
endif()
...
if(VCPKG_TARGET_IS_IOS)
	...
	elseif(VCPKG_TARGET_ARCHITECTURE STREQUAL "arm")
		message(FATAL_ERROR
			"You can build for arm up to iOS 10 but ffmpeg can only be built for iOS 11.0 and later.
                            Did you mean arm64?"
		)
	elseif(VCPKG_TARGET_ARCHITECTURE STREQUAL "x86")
		message(FATAL_ERROR
			"You can build for x86 up to iOS 10 but ffmpeg can only be built for iOS 11.0 and later.
                            Did you mean x64"
		)
	endif()
	...
	elseif(VCPKG_OSX_DEPLOYMENT_TARGET
		LESS
		11.0
	) # nowadays ffmpeg needs to be built for ios 11.0 and later
		message(FATAL_ERROR
			"ffmpeg can be built only for iOS 11.0 and later but you set VCPKG_OSX_DEPLOYMENT_TARGET to
	...
	set(OPTIONS
		"${OPTIONS} --extra-cflags=--target=${vcpkg_target_arch}-apple-ios${vcpkg_osx_deployment_target}${simulator}"
	)
	set(OPTIONS
		"${OPTIONS} --extra-ldflags=--target=${vcpkg_target_arch}-apple-ios${vcpkg_osx_deployment_target}${simulator}"
	)
	...
	if(OR OR) # if it's not a path
		...
		if(sdk_path)
			...
			message(FATAL_ERROR
				"Can't determine ${CMAKE_OSX_SYSROOT} SDK path. Error: ${xcrun_error}"
			)
		endif()
	endif()
...
if(VCPKG_DETECTED_MSVC)
	...
	string(APPEND
		OPTIONS_RELEASE
		" --extra-cflags=${FFMPEG_CRT_PREFIX} --extra-cxxflags=${FFMPEG_CRT_PREFIX}"
	)
	# Append Debug flags (adding the 'd' suffix for the debug runtime)
	string(APPEND
		OPTIONS_DEBUG
		" --extra-cflags=${FFMPEG_CRT_PREFIX}d --extra-cxxflags=${FFMPEG_CRT_PREFIX}d"
	)
endif()
...
separate_arguments(standard_libraries
	NATIVE_COMMAND
...
if(NOT VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "release")
	if(VCPKG_DETECTED_MSVC)
		set(OPTIONS_RELEASE
			"${OPTIONS_RELEASE} --extra-ldflags=-libpath:\"${CURRENT_INSTALLED_DIR}/lib\""
		)
	else()
		set(OPTIONS_RELEASE
			"${OPTIONS_RELEASE} --extra-ldflags=-L\"${CURRENT_INSTALLED_DIR}/lib\""
		)
	endif()
	...
	string(REGEX
		...
		"-arch [A-Za-z0-9_]+"
		""
		VCPKG_COMBINED_C_FLAGS_RELEASE_SANITIZED
		"${VCPKG_COMBINED_C_FLAGS_RELEASE}"
	)
	...
	string(REGEX
		...
		"-arch [A-Za-z0-9_]+"
		""
		VCPKG_COMBINED_SHARED_LINKER_FLAGS_RELEASE_SANITIZED
...
if(NOT VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
	if(VCPKG_DETECTED_MSVC)
		set(OPTIONS_DEBUG
			"${OPTIONS_DEBUG} --extra-ldflags=-libpath:\"${CURRENT_INSTALLED_DIR}/debug/lib\""
		)
	else()
		set(OPTIONS_DEBUG
			"${OPTIONS_DEBUG} --extra-ldflags=-L\"${CURRENT_INSTALLED_DIR}/debug/lib\""
		)
	endif()
	...
	string(REGEX
		...
		"-arch [A-Za-z0-9_]+"
		""
		VCPKG_COMBINED_C_FLAGS_DEBUG_SANITIZED
		"${VCPKG_COMBINED_C_FLAGS_DEBUG}"
	)
	...
	string(REGEX
		...
		"-arch [A-Za-z0-9_]+"
		""
		VCPKG_COMBINED_SHARED_LINKER_FLAGS_DEBUG_SANITIZED
...
append_dependencies_from_libs(FFMPEG_DEPENDENCIES_RELEASE
	LIBS "${FFMPEG_PKGCONFIG_LIBS_RELEASE}"
)
append_dependencies_from_libs(FFMPEG_DEPENDENCIES_DEBUG
	LIBS "${FFMPEG_PKGCONFIG_LIBS_DEBUG}"
)
...
function(extract_regex_from_file out)
	...
	if(contents MATCHES "${arg_REGEX}")
		if(NOT CMAKE_MATCH_COUNT EQUAL 1)
			message(FATAL_ERROR
				"Could not identify match group in regular expression \"${arg_REGEX}\""
			)
		endif()
	else()
		if(arg_MAJOR)
			...
			if(contents MATCHES "${arg_REGEX}")
				...
				message(WARNING
					"Could not find line matching \"${arg_REGEX}\" in file \"${arg_FILE_WITHOUT_EXTENSION}_major.h\""
				)
			endif()
		else()
			message(WARNING
				"Could not find line matching \"${arg_REGEX}\" in file \"${arg_FILE_WITHOUT_EXTENSION}.h\""
			)
		endif()
	endif()
...
# Handle copyright
file(STRINGS "${CURRENT_BUILDTREES_DIR}/build-${TARGET_TRIPLET}-rel-out.log"
	LICENSE_STRING
	REGEX "License: .*"
	LIMIT_COUNT 1
)
